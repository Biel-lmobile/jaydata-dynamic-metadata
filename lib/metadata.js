...
var Metadata = (function () {
    ...
    Metadata.prototype.createEntityType = function (entitySchema, namespace) {
        ...
        if (entityFullName.startsWith("Default") && entityFullName.includes("ExtensionValues")) {
            entityFullName = entityFullName.replace("Default.", "");
        }
        entitySchema.properties.map(prop => {
            if (prop.name === "ExtensionValues") {
                prop.type = prop.type.replace("Default.", "")
            }
        })
